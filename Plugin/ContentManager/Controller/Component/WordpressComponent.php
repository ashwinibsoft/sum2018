...
class WordpressComponent extends Component {
	 ...
	 * @var string
	 */
	var $__wp_database = 'balance-physio';
	 ...
	 * @var string
	 */
	var $__wp_username = 'balance-physio';
	 ...
	 * @var string
	 */
	var $__wp_password = 'balance@physio';
	...
	function __query_posts() {
		...
		$db_query = "SELECT ID, post_date, post_content, post_title, post_name, guid,comment_count
                FROM wp_posts
	...
	function __process_posts($posts_query) {
		for($i = 0; $i < $posts_query['num_rows']; $i++) {
			...
			$posts[$i]['comment_count'] = mysql_result($posts_query['query'], $i, "comment_count");
			//$posts[$i]['author'] = mysql_result($posts_query['query'], $i, "wp_users.post_author");
			// Date
