...
$html ='<div style="padding:6%; margin:0px auto; width:1110px;">
    ...
    if(!empty($suppliers)){
		foreach ( $suppliers as $supplier ){
			...
			/*if($supplier['Supplier']['dob']){
				$dob  = date("m/d/Y", strtotime($supplier_info['Supplier']['dob']));
			}else{
				$dob = 'N/A';
			}*/
		...
		<td style="padding:10px;border-right: 1px solid #000;border-bottom: 1px solid #000;">'.$supplier['Supplier']['dob'].'</td>
		<td style="padding:10px;border-right: 1px solid #000;border-bottom: 1px solid #000;">'.$supplier['Supplier']['contact_number'].'</td>
...
$dompdf->stream($nb_info['NewBuyer']['first_name'].'_'.$nb_info['NewBuyer']['last_name'].'_supplier_list('.$currntdate.').pdf');
 die;
?>
