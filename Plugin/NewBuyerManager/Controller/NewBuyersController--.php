...
Class NewBuyersController extends NewBuyerManagerAppController{
	...
	//	echo $member_id; die;
		...
		//echo '<pre>'; print_r($result['existingBuyer']); die;
	...
	function export_list()
	{
		...
		$currntdate=date('d/m/Y-H:i:s');  
		$fileName=$user['NewBuyer']['first_name'].'-supplier_list-'.$currntdate.'.xls';
		 ...
		 // pr($this->request->data); die;
		  ...
		  foreach($this->request->data['tier'] as $test){
	       ...
	       //pr($tier_arr); die;
			...
			$categories = $this->QuestionCategorie->find('all',array('fields'=>array('QuestionCategorie.name','QuestionCategorie.id'),'conditions'=>array('QuestionCategorie.id'=>$category)));
