...
Class NewBuyersController extends NewBuyerManagerAppController{
	...
	//	echo $member_id; die;
		//print_r($this->Session->read('MemberAuth'));die;
		  ...
		  foreach($all_requests as $r_all)
				{
					...
					//$this->loadmodel('ExistingBuyerManager.ExistingBuyer');
					 
				}
			
		}
		$existing_buyer_id = '';
		foreach($result['existingBuyer'] as $key => $value){
			if($existing_buyer_id == ''){
				$existing_buyer_id .= $key;
				}else{
			$existing_buyer_id .= ','.$key;
		}
			}
		//print_r($existing_buyer_id);exit;
		if($existing_buyer_id!=""){
		$q = "SELECT `feedback_requests`.`request_status`  FROM `feedback_responses` 
LEFT JOIN  `feedback_requests` ON `feedback_responses`.`request_id` =  `feedback_requests`.`id`
WHERE `feedback_responses`.`existing_buyer_id` IN ($existing_buyer_id)";
        $db = ConnectionManager::getDataSource('default');
        $existing_buyer_status = $db->query($q);
	    }
//echo "<pre>";print_r($existing_buyer_status);
		//~ $existing_buyer_status = $this->FeedbackRequest->query('SELECT *  FROM `feedback_responses` WHERE request_id IN ('.$existing_buyer_id.')');
			//echo $existing_buyer_id;
		$existing_buyer_name = $result['existingBuyer'];
		//echo '<pre>'; print_r($existing_buyer_name); die;
