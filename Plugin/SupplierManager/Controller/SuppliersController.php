...
Class SuppliersController extends SupplierManagerAppController{
	public $uses = array('SupplierManager.Supplier','SupplierManager.NewbuyerExist','ContentManager.Page','Country','SupplierManager.SupplierBuyer','NewBuyerManager.NewBuyer','ExistingBuyerManager.ExistingBuyer','SupplierManager.FeedbackResponse','NewBuyerManager.NewBuyerQuestion','ExistingBuyerManager.EbLoginDetail','SupplierManager.FeedbackRequest','SupplierManager.FeedbackRequest2','SupplierManager.FeedbackTemp','SupplierManager.Payment','QuestionManager.Question');
	...
	foreach($in_progress as $getVal){
		$temp_allexpire = $this->EbLoginDetail->find('all',array('conditions'=>array('EbLoginDetail.request_id'=>$getVal['FeedbackRequest']['id'],'EbLoginDetail.eb_status !=' =>6,'NOT'=>array('EbLoginDetail.is_link_expire' =>array(1,3))),'fields'=>array('EbLoginDetail.existing_buyer_id'),'Limit'=>5));
		  if(!empty($temp_allexpire)){ 
		   foreach($temp_allexpire as $pgrs){ 
			   $exist_prgs_id [] = $pgrs['EbLoginDetail']['existing_buyer_id'];
			  }
	        }	      
		}
		return $exist_prgs_id;
	}
	...
	function admin_index($search=null,$limit=10){
		...
		foreach($suppliers as $key=>$value){
			if($inval==""){
				$inval = $value['Supplier']['id'];
			}else{
				$inval = $inval.",".$value['Supplier']['id'];
			}
		   }
	    $sql="SELECT supplier_buyers.supplier_id,supplier_buyers.category FROM suppliers LEFT JOIN supplier_buyers ON suppliers.id=supplier_buyers.supplier_id WHERE suppliers.id IN($inval)";
		$category = $this->Supplier->query($sql);
		...
		foreach($data as $value){
			if($value!='0'){
			  ...
			  $this->FeedbackRequest->bindModel(array('hasMany' => array('FeedbackResponse'=>array(
					'foreignKey'=>'request_id','dependent'=>true))), false);
			  $this->ExistingBuyer->bindModel(array('hasMany' => array('EbLoginDetail'=>array(
					'foreignKey'=>'existing_buyer_id','dependent'=>true))), false);
			  $this->Supplier->bindModel(array('hasMany' => array('FeedbackRequest'=>array('dependent'=>true),'Payment'=>array('dependent'=>true),'ExistingBuyer'=>array('dependent'=>true),)), false);
			  $supplier=$this->Supplier->find('first',array('conditions'=>array('Supplier.id'=>$value)));
	...
	public function registration_success(){
		...
		$message = '<p class="txt-bold">Your account has been created successfully.</p><p>Please check your mail inbox to activate your account.<br>Also you can contact us for any query or suggestion.</p>'; 
	
		//$url = Router::url('/contact-us');
		$url = Router::url(array('plugin'=>'supplier_manager','controller'=>'suppliers','action'=>'login'), true );
	...
	public function edit_profile(){
		...
		if(!empty($this->request->data['Supplier']['form'])) {
			...
			//$this->request->data['Supplier']['company_name']=strtoupper($this->request->data['Supplier']['company_name']);
				...
				$this->request->data['Supplier']['company_name']= ucfirst($this->request->data['Supplier']['company_name']);
			}	
			...
			//$this->Session->setFlash(__('Profile has been updated successfully.'),'default',array(),'success');
				...
				$this->redirect(array('action'=>'add_new_buyer'));
					...
					$this->redirect(array('action'=>'dashboard/#dashboardPro'));	
			...
			//$data['Supplier']['dob']=date('dd/mm/yyyy',strtotime($data['Supplier']['dob']));
	...
	public function required_feedback($id=null) {
		...
		/*=====================GETTING TOTAL NUMBER OF FEEDBACK  REQUIRED FOR NEWBUYER============================*/
		...
		/*=====================GETTING COMPLETED RESPONSE EXISTING BUYER============================*/
		
        $sql ="SELECT `existing_buyer_id` FROM `feedback_responses` c  INNER JOIN `existing_buyers`  ca  ON ca.`id` = c.`existing_buyer_id` WHERE c.`response_status` =\" 2 \" AND ca.`supplier_id` = \"$loguser_id \" LIMIT 5";

		$val = $this->FeedbackResponse->query($sql);

		//$sql = "SELECT `existing_buyer_id` FROM `feedback_responses` c  INNER JOIN `existing_buyers`  ca  ON ca.`id` = c.`existing_buyer_id` WHERE  ca.`supplier_id` = \"$loguser_id \" ";
		
		/*$val_test = $this->FeedbackResponse->query($sql);*/
		...
		if(!empty($record)) { 
		foreach($record as $getDetails){
			$userDetails[] = $this->ExistingBuyer->find('first',array('fields'=>array('id','first_name','last_name','org_name'),'conditions'=>array('ExistingBuyer.id'=>$getDetails)));
		  }
		}
		
		if(count($record) >= $tot_req_feed){
		  $req = 1;	
		}else{
			    $req = 0;
		}
		
		$this->set('required_second_options', $req);

		/*=======================GETTING ON GOING RESPONSE EXISTING BUYER============================*/

		$in_progress = $this->FeedbackRequest->find('all',array('conditions'=>array('FeedbackRequest.supplier_id'=>$loguser_id),'fields'=>array('id')));
		
		if(!empty($in_progress)) {
		
		$inprogressExistingBuyer = array_column($in_progress,'FeedbackRequest');
		
		$forFilterData = array_column($inprogressExistingBuyer , 'id');
		
		$finaFilterData = array_unique($forFilterData);
		
		$this->loadModel('ExistingBuyerManager.EbLoginDetail');

        $getExistingBuyerDetails = $this->EbLoginDetail->find('all',array('conditions'=>array('EbLoginDetail.request_id IN'=>$finaFilterData,'EbLoginDetail.eb_status !=' =>6,'NOT'=>array('EbLoginDetail.is_link_expire' =>array(1,3))),'fields'=>array('ExistingBuyer.id','ExistingBuyer.first_name','ExistingBuyer.last_name','ExistingBuyer.org_name'),'limit' => 5));

        foreach ($getExistingBuyerDetails as $existingBuyerDetails) {
        	$exbuyer_retails [] = $existingBuyerDetails['ExistingBuyer'];
         }
        }
		
        //echo '<pre>'; print_r($exbuyer_retails); exit;

		/*//foreach($in_progress as $getVal){ 
		foreach($finaFilterData as $getVal){ 
			
		 $temp_allexpire = $this->EbLoginDetail->find('all',array('conditions'=>array('EbLoginDetail.request_id'=>$getVal,'EbLoginDetail.eb_status !=' =>6,'NOT'=>array('EbLoginDetail.is_link_expire' =>array(1,3))),'fields'=>array('ExistingBuyer.id','ExistingBuyer.first_name','ExistingBuyer.last_name','ExistingBuyer.org_name'),'limit' => 5));
		   
		 if(!empty($temp_allexpire)){
			  $check_expire[] = $temp_allexpire;
			  }
		}
		
		//echo '<pre>'; print_r($check_expire); exit;
	   foreach($check_expire as $check_expires){
		
		  foreach($check_expires as $inprogress_buyer){
			  
			$exbuyer_retails [] = array('first_name'=>$inprogress_buyer['ExistingBuyer']['first_name'],'last_name'=>$inprogress_buyer['ExistingBuyer']['last_name'],'org_name'=>$inprogress_buyer['ExistingBuyer']['org_name']) ; 
			  }
		}*/ 
		
	    $this->set('in_progress_exbuyer',$exbuyer_retails);
		$this->set('recordDetails', $userDetails);	
	...
	// End  for required feedback	
		...
		$val = $this->FeedbackResponse->query("SELECT `existing_buyer_id` FROM `feedback_responses` c
              INNER JOIN `existing_buyers`  ca  ON ca.`id` = c.`existing_buyer_id` WHERE c.`response_status` =\" 2 \" AND ca.`supplier_id` = \"$loguser_id \" ");
       ...
       // $alerady_response = $this->FeedbackResponse->find('list',array('fields'=>array('FeedbackResponse.existing_buyer_id'),'conditions'=>array('FeedbackResponse.response_status'=>2)));
       
       if(empty($eb)){
		   
		 $this->Session->setFlash(__('Please add existing buyer first.'),'default',array(),'error');
...
// for exired existing buyer
	...
	// for required feedback
		...
		/*$this->paginate['ExistingBuyer'] = array(
		 ...
		 $exist_pending = $this->__exist_buyer_pending($loguser_id);
		 
		 $rest = max($req_fed)-count($exist_pending);
		 
		 if($rest >0 ){
			$lst_ex = $this->ExistingBuyer->find('list',array('fields'=>array('ExistingBuyer.id'),'conditions'=>array('ExistingBuyer.supplier_id'=>$loguser_id,'ExistingBuyer.status'=>1,'ExistingBuyer.replace'=>3),'order'=>array('ExistingBuyer.id DESC'),'limit' => $rest));
			
			$exist_pending = array_merge($exist_pending,array_values($lst_ex));
			  
			 }
		
		 $exist_both = array_merge($exist_pending,array_values($response_id));
		 
		 $rest_both = max($req_fed)-count($exist_both);
		 
		  if($rest_both >0 ){
			$lst_ex_both = $this->ExistingBuyer->find('list',array('fields'=>array('ExistingBuyer.id'),'conditions'=>array('ExistingBuyer.supplier_id'=>$loguser_id,'ExistingBuyer.status'=>1,'ExistingBuyer.replace'=>4),'order'=>array('ExistingBuyer.id DESC'),'limit' => $rest_both));
		 ...
		 if($_SESSION['nb_button']){
				 
			if(count($_SESSION['nb_button'])>1){
				
					$select_exist_buyer_id = $exist_both;
					
				}else if($_SESSION['nb_button'][0]== 'nb_button1'){
					
					$select_exist_buyer_id = $response_id;
					
				} else if($_SESSION['nb_button'][0]=='nb_button2'){
		...
		if(empty($select_exist_buyer_id)){
			
		$new_exist_id=$this->ExistingBuyer->find('all',array('conditions'=>array('ExistingBuyer.supplier_id'=>$loguser_id,'ExistingBuyer.status'=>1),'order' => array('ExistingBuyer.id'=>'DESC'),'limit' => 5,'fields'=>array('id')));
		
		foreach($new_exist_id as $insert_id){
			    $val2[] = $insert_id['ExistingBuyer']['id'];	
			}
			
		 if(!empty($response_id)){	
			$select_exist_buyer_id = $response_id;
		   }else {
			   $select_exist_buyer_id = $val2;
		   }			
		}
...
// for reference num and category update 	
	...
	//echo "<pre>"; print_r($custom_variable);die;					
				...
				//$myPaypal->addField('cmd', '_s-xclick');
				...
				//echo "<pre>"; print_r($myPaypal->fields);die;
...
foreach($all_req_id as  $_all_req_id){
	...
	public function dashboard(){
	    ...
	    foreach($eb as $find_exbuyer){	
		 $val_exbuyer = $this->EbLoginDetail->find('all',array('conditions'=>array('EbLoginDetail.existing_buyer_id'=>$find_exbuyer['ExistingBuyer']['id'],'EbLoginDetail.eb_status !=' =>6,'NOT'=>array('EbLoginDetail.is_link_expire' =>array(1,3)))));
			
		 if(!empty($val_exbuyer)){
		    foreach($val_exbuyer as $val_exbuyer){
				  $pending_arr[] =   $val_exbuyer['ExistingBuyer']['id'];
				  }
			   }
		  }
		 if(!empty($pending_arr)){	
			$this-> set('pending_exbuy',$pending_arr);
		 }
		...
		$this->FeedbackRequest2->bindModel(array('hasMany' => array('Payment'=>array('order' => 'Payment.id DESC'))));
		
		$this->paginate['FeedbackRequest2'] = array('conditions'=>array('FeedbackRequest2.supplier_id'=>$loguser_id),
		  'order' => array('FeedbackRequest2.id'=>'DESC'), 'limit' => 500);
		
		$requests = $this->paginate('FeedbackRequest2');
     ...
     /*==========START DISABLE READY TO FORWARD FEEDBACK BUTTON, IF SUPPLIER ONCE SEND FEEDBACK TO NEWBUYER==============*/
     
     foreach($eb_list as $_eb_list){ 
		 $list[] = $this->FeedbackRequest2->find('all',array( 'conditions'=>array('FeedbackRequest2.id'=>$_eb_list['EbLoginDetail']['request_id'])));
	  }
    
     $array_new = [];
     foreach ($list as $fb_list) {
		 foreach ($fb_list as $_val)  {   
			   $id_by=$_val['FeedbackRequest2']['id'];
		       $res_feed = json_decode($_val['FeedbackRequest2']['is_report_sent']);
		       $test_array=array();
			      foreach($res_feed as $key=>$val){
			                     $test_array[$key]=$val;
		                    }
		        $array_new[$id_by]=$test_array;
	       }
	    }
	
	     $buyer_id=[];
	  
	     foreach ($requests as $fb_list)  {
		
				$pid= (json_decode($fb_list['FeedbackRequest2']['selected_new_b_exist']));
				
				$ids= (json_decode($fb_list['FeedbackRequest2']['new_buyers']));
				
				foreach($ids as $id) {
					
					foreach($pid->$id as $sid) {
						
						$buyer_id[$id]['feedback_details'][]=self::_check_expiry($fb_list['FeedbackRequest2']['id'],$sid);
					}
			
			   if(!empty($id)){
						
				$buyer_id[$id]['feedback'][]=$fb_list['FeedbackRequest2']['id'];
						
				//$buyer_id[$id]=$fb_list['FeedbackRequest']['id'];
						
				foreach($fb_list['Payment'] as $pay) {
					
					 $buyer_id[$id]['payment'][]=$pay;
							
						}
					
			$this->SupplierBuyer->bindModel(array('belongsTo' => array('Country' => array('foreignKey' => false,'conditions' => array('NewBuyer.country = Country.country_code_char2','NewBuyer.country = Country.country_code_char2')))));	
				
			$buyer_id[$id]['data'] = $this->SupplierBuyer->find('first',array('conditions'=>array('SupplierBuyer.supplier_id'=>$loguser_id,'NewBuyer.id'=>$id), 'order'=>array('SupplierBuyer.id'=>'DESC')));
				
			//$buyer_id[$id]['feedback_details']=self::_check_expiry($fb_list['FeedbackRequest']['id']);				
	   ...
	   $this->set("buyers",$buyer_id);
	  
       $this->set('our_feedback', $array_new);
        
   /*==============END DISABLE READY TO FORWARD FEEDBACK BUTTON, IF SUPPLIER ONCE SEND FEEDBACK TO NEWBUYER================*/	

        $ques_cat=array('t1'=>'Tier1','t2'=>'Tier2','t3'=>'Tier3','t4'=>'Tier4');

        $this->set('ques_cat', $ques_cat);
